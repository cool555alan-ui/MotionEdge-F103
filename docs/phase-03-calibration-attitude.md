...
传感器坐标系定义为X轴向前、Y轴向右、Z轴向上；Roll绕X轴，Pitch绕Y轴。MPU6050
...
当前没有连接硬件，因此真实零偏、校准稳定性、姿态精度、噪声、温漂、长期漂移、
USART1 CSV输出和ST-LINK烧录均未实机验证。

## 实机验证清单

1. 检查PB6/PB7上拉、模块电源和地址；
2. 静止保持至少5秒，确认500样本校准完成；
3. 六个静止朝向检查加速度幅值和符号；
4. 已知角度治具检查Roll/Pitch方向和误差；
5. 长时间静止记录噪声、温漂和陀螺漂移；
6. 串口采集CSV并运行`validate`与`summary`；
7. 使用ST-LINK完成烧录、复位和调试。
