...
void TestRtosMonitor_Run(TestContext_t *context)
{
    ...
    TEST_EXPECT(context, RTOS_TELEMETRY_STACK_BYTES == 1024U);
    TEST_EXPECT(context, RTOS_HEALTH_STACK_BYTES == 1280U);
    TEST_EXPECT(context, RTOS_TOTAL_APPLICATION_STACK_BYTES == 4608U);
    TEST_EXPECT(context, RTOS_COMMAND_QUEUE_CAPACITY == 8U);
    TEST_EXPECT(context, RTOS_COMMAND_MAX_PAYLOAD_SIZE == 10U);
    TEST_EXPECT(context, sizeof(command) <= 20U);
    TEST_EXPECT(context,
