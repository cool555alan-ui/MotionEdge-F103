...
static uint8_t s_identity = MPU6500_WHO_AM_I_VALUE;
...
static bool MockRead(uint8_t address_7bit,
                     ...
                     size_t length)
{
    if (!s_read_success || (address_7bit != MPU6500_ADDRESS_AD0_LOW) ||
        (data == NULL))
    {
        return false;
    }
...
{
    Mpu6500_t device = {0};
    Mpu6500RawData_t raw_data = {0};
    Mpu6500ScaledSample_t scaled_sample = {0};
    uint8_t identity = 0U;
    ...
    TEST_EXPECT(context,
                Mpu6500_Init(NULL,
                             ...
                             MockRead,
                             MockWrite) == MPU6500_ERROR_INVALID_ARG);
    TEST_EXPECT(context,
                Mpu6500_Init(&device, 0x67U, MockRead, MockWrite) ==
    ...
    TEST_EXPECT(context,
                Mpu6500_Init(&device,
                             ...
                             MockWrite) == MPU6500_ERROR_INVALID_ARG);
    TEST_EXPECT(context,
                Mpu6500_ReadWhoAmI(&device, &identity) ==
                    MPU6500_ERROR_NOT_INITIALIZED);
    TEST_EXPECT(context,
                Mpu6500_Init(&device,
                             MPU6500_ADDRESS_AD0_LOW,
                             MockRead,
                             MockWrite) == MPU6500_OK);
    ...
    TEST_EXPECT(context, s_write_address == MPU6500_ADDRESS_AD0_LOW);
    ...
    TEST_EXPECT(context, Mpu6500_ReadRaw(&device, &raw_data) == MPU6500_OK);
    ...
    TEST_EXPECT(context, Mpu6500_ReadWhoAmI(&device, &identity) == MPU6500_ERROR_BUS);
    TEST_EXPECT(context, Mpu6500_ReadRaw(&device, &raw_data) == MPU6500_ERROR_BUS);
    s_read_success = true;
    s_write_success = false;
    TEST_EXPECT(context, Mpu6500_Wake(&device) == MPU6500_ERROR_BUS);
    TEST_EXPECT(context,
                Mpu6500_ReadRaw(&device, NULL) == MPU6500_ERROR_INVALID_ARG);
    raw_data =
        (Mpu6500RawData_t){16384, -16384, 8192, 131, -131, 262};
    TEST_EXPECT(context, Mpu6500_ScaleRaw(&raw_data, &scaled_sample));
    ...
    TEST_EXPECT(context, !Mpu6500_ScaleRaw(NULL, &scaled_sample));
}
